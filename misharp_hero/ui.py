...
def page_explore():
    st.title("상품 탐색")
    st.caption(
        "평일 정오 전 신상페이지 순서와 비교해 상단에 새롭게 오픈된 상품을 출시로 판단하고, "
    ...
    s2.metric("이번 확인 신규오픈", f"{int(status.get('added') or 0):,}개")
    ...
    st.caption(
        "상품탐색 등록은 '정오 전 대비 신상 상단 신규오픈 + Cafe24 판매/진열 확인'이 모두 충족된 상품만 자동 처리합니다."
    )
