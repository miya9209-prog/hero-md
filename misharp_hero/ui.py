...
def page_radar():
    ...
    show = launches.copy()
    show["경과"] = ((now - pd.to_datetime(show["launch_at"])).dt.total_seconds()/3600).clip(lower=0).round(1)
    show["CVR"] = show["cvr"].apply(_pct)
    show["RPV"] = show["rpv"].apply(_money)
    cols = ["product_name","경과","views","order_count","qty","revenue","CVR","RPV","hero_score","hero_grade","diagnosis"]
    st.subheader("상품 현황")
    st.dataframe(
        show[cols].rename(columns={
            "product_name":"상품명","views":"조회","order_count":"판매건수",
            "qty":"판매수량","revenue":"매출","hero_score":"HERO Score",
            "hero_grade":"등급","diagnosis":"자동진단"
        }),
...
def page_schedule():
    ...
    if data.empty:
        ...
        cols = [c for c in [
            "exposure_plan_at","supplier_product_name","supply_price","tentative_name",
            "focus_candidate","hero_dna","supplier_name","order_plan_at","status",
            "prelaunch_score","product_no","cafe24_product_name"
        ] if c in show.columns]
        st.dataframe(show[cols].rename(columns={
            "exposure_plan_at":"노출예정","supplier_product_name":"공급사상품명",
            "supply_price":"공급가","tentative_name":"가칭상품명",
            "focus_candidate":"FOCUS","hero_dna":"HERO DNA","supplier_name":"소싱처",
            "order_plan_at":"발주예정","status":"상태","prelaunch_score":"예상 HERO Score",
            "product_no":"Cafe24 상품번호","cafe24_product_name":"Cafe24 상품명"
    ...
    with st.expander("신상품 후보 추가"):
        ...
        if st.button("후보 추가", type="primary") and name:
            score = prelaunch_score(
                focus=focus, md_score=md, season_score=season,
                reorder_score=reorder, content_score=content
            )
            ...
            add_candidate({
                "product_id":pid, "supplier_product_name":name,
                "tentative_name":tentative, "exposure_plan_at":datetime.combine(exposure, datetime.min.time()),
                "focus_candidate":focus, "hero_dna":dna, "md_score":md,
                "season_score":season, "reorder_score":reorder, "content_score":content,
                "prelaunch_score":score, "status":"후보"
            })
...
def page_48h():
    ...
    show["RPV"] = show["rpv"].apply(_money)
    cols = ["product_name","launch_at","48H 마감","views","order_count","qty","CVR","수량전환율","RPV","revenue","hero_score","hero_grade","diagnosis","hero_manual","review_manual"]
    st.dataframe(show[cols].rename(columns={
        "product_name":"상품명","launch_at":"노출시각","views":"조회",
        "order_count":"판매건수","qty":"판매수량","revenue":"매출",
        "hero_score":"HERO Score","hero_grade":"자동등급","diagnosis":"자동진단",
        "hero_manual":"MD HERO","review_manual":"재검토"
    }), use_container_width=True, hide_index=True)
    st.caption("API 연동 전 엑셀 이관값의 판매건수는 판매수량을 임시 대체값으로 사용합니다. Analytics 연동 후 정확한 판매건수로 갱신됩니다.")
...
def page_hero_list():
    ...
    st.dataframe(show.rename(columns={
        "month":"월","product_name":"상품명","supplier_product_name":"공급사상품명",
        "hero_status":"HERO","qty":"판매수량","revenue":"매출",
        "margin_rate":"공헌이익률","gross_profit":"공헌이익액",
        "keep_status":"유지여부","revenue_rank":"매출순위",
        "profit_rank":"이익순위","margin_rank":"이익률순위","note":"비고"
    }), use_container_width=True, hide_index=True)
...
def page_actions():
    ...
    if not data.empty:
        st.dataframe(data.rename(columns={
            "product_name":"상품명","issue_type":"이슈","action_text":"조치",
            "owner":"담당","due_at":"기한","status":"상태","team":"팀","note":"메모"
        }), use_container_width=True, hide_index=True)
    ...
    with st.form("action_add"):
        ...
        if submitted and action:
            ...
            add_action({"product_name":product,"issue_type":issue,"action_text":action,"owner":owner,"team":team,"status":"대기"})
            st.success("추가했습니다.")
            st.rerun()
...
def page_import():
    ...
    f = st.file_uploader("미샵 핵심업무 실행서식 업로드", type=["xlsx","xlsm"], key="md")
    ...
    sera = df("SELECT report_name, product_no, product_name, views, orders, qty, revenue, opv, espv, imported_at FROM sera_metrics ORDER BY imported_at DESC LIMIT 100")
    if not sera.empty:
        st.subheader("최근 SERA 적재")
        st.dataframe(sera, use_container_width=True, hide_index=True)
...
def page_settings():
    ...
    if CAFE24_MALL_ID and CAFE24_CLIENT_ID and CAFE24_REDIRECT_URI:
        try:
            st.link_button("Cafe24 Admin 권한 승인 열기", AdminOAuth().authorize_url())
        except Exception as e:
            st.error(str(e))
        code = st.text_input("승인 후 Admin code 붙여넣기", type="password")
        ...
        st.warning("CAFE24_MALL_ID / CLIENT_ID / REDIRECT_URI를 먼저 설정하세요.")

    st.divider()
    st.subheader("Cafe24 Analytics OAuth")
    if AnalyticsOAuth().configured():
        st.link_button("Analytics 권한 승인 열기", AnalyticsOAuth().authorize_url())
        acode = st.text_input("승인 후 Analytics code 붙여넣기", type="password")
